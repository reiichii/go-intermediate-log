...
func TestArticleListHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, url, nil) // http.Request型を作ってくれる

			res := httptest.NewRecorder() // ResponseRecorder 構造体は、http.ResponseWriter インターフェースを実装している
...
func TestArticleDetailHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// gorilla/mux のルータを通さないとハンド ラ内で使われている mux.Vars 関数が正しく動かないため、gorilla/muxのルータを用意
			r := mux.NewRouter()
			// テストで使うパスとハンドラの対応関係を登録
